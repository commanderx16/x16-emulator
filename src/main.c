...
usage()
{
	...
	"(C)2019,2020,2021 Michael Steil et al.\n"
	"All rights reserved. License: 2-clause BSD\n\n"
	...
	"-joy1\n"
	"\tEnable binding a gamepad to SNES controller port 1\n"
	"-joy2\n"
	"\tEnable binding a gamepad to SNES controller port 2\n"
	"-joy3\n"
	"\tEnable binding a gamepad to SNES controller port 3\n"
	"-joy4\n"
	"\tEnable binding a gamepad to SNES controller port 4\n"

	"-sound <output device>\n"
	"\tSet the output device used for audio emulation\n"
	"\tIf output device is 'none', no audio is generated\n"
	...
	"-rtc\n"
	"\tSet the real-time-clock to the current system time and date.\n"
#ifdef TRACE
	"-trace [<address>]\n"
...
//
// returns keymap index from name; returns -1 if not found
//
...
//
// process parms from INIfile and CLI (in that order)
//
void processParms(int argc, char **argv, Parms *parms) {
	...
	if(iniDict) {
		...
		parms->nvram_path= (char *)iniparser_getstring(iniDict, "main:nvram", NULL);
		...
		parms->set_system_time= 1 == iniparser_getboolean(iniDict, "main:rtc", 0);
		...
		for(int joyIdx= 0; joyIdx<NUM_JOYSTICKS; joyIdx++) {
			...
			Joystick_slots_enabled[joyIdx]= 1 == iniparser_getboolean(iniDict, joykey, 0);
		}
...
main(int argc, char **argv)
{
	...
	if (parms.nvram_path) {
		SDL_RWops *f = SDL_RWFromFile(parms.nvram_path, "rb");
		if (f) {
	...
	rtc_init(parms.set_system_time);
...
//			}
		...
		if (new_frame) {
			if (nvram_dirty && parms.nvram_path) {
				SDL_RWops *f = SDL_RWFromFile(parms.nvram_path, "wb");
				if (f) {
