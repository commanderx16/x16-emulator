...
video_init(uint8_t *in_chargen, int window_scale)
{
	...
	if(record_gif)
	{
		record_gif = GifBegin(&gif_writer, gif_path, SCREEN_WIDTH,
              SCREEN_HEIGHT, 1, 8, false);
	}
...
video_update()
{
	...
	if(record_gif)
	{
		record_gif = GifWriteFrame(&gif_writer, framebuffer, 
			SCREEN_WIDTH, SCREEN_HEIGHT, 1, 8, false);
		if(!record_gif)
			GifEnd(&gif_writer);
	}
